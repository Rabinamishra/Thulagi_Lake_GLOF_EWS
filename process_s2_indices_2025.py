...
BASE = Path("data/processed/sentinel2/2025_12_30")
...
if quant_match is None:
    raise RuntimeError(
        "QUANTIFICATION_VALUE not found in MTD_MSIL1C.xml"
    )

if offset_match is None:
    raise RuntimeError(
        "RADIO_ADD_OFFSET for B03 not found in MTD_MSIL1C.xml"
    )
...
with rasterio.open(B08) as src:
    ...
    if src.width != width or src.height != height:
        raise RuntimeError(
            "B03 and B08 do not have matching dimensions."
        )

    if src.transform != transform:
        raise RuntimeError(
            "B03 and B08 do not share the same grid."
        )
...
ndwi_file = OUTPUT / "S2B_20251230_NDWI_TOA.tif"
...
mndwi_file = OUTPUT / "S2B_20251230_MNDWI_TOA.tif"
